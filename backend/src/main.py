...
出力: \n
"""     
            response = chatbot.create([chat.Message('system', prompt).asdict()])
            print(response)
            return response
        ...
        return [{
            ...
            "tsuriScore": 100 - int((content := score_item(video))[content.find("\"納得感\": ") + len("\"納得感\": "):][:2].strip()),
            "tsuriReport": {
                "try_parse_json": try_parse_json(content),
                "raw": content
            }
